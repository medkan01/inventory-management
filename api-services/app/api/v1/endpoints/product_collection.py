...
def get_collections(
    ...
    - **limit**: Nombre maximum d'éléments à retourner (défaut: DEFAULT_PRODUCT_COLLECTION_LIMIT, max: MAX_PRODUCT_COLLECTION_LIMIT)
    ...
    collections = product_collection_service.get_all_collections(db=db, skip=skip, limit=limit)
    return collections
...
def get_collection_by_slug(
    ...
    Nécessite une authentification.
    """
    collection = product_collection_service.get_collection_by_slug(db=db, slug=collection_slug)
...
@router.post("/", response_model=ProductCollectionResponse, status_code=status.HTTP_201_CREATED)
def create_collection(
    ...
    """
    try:
        collection = product_collection_service.create_collection(db=db, collection_in=collection_in)
        return collection
